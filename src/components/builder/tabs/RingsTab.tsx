...
export function RingsTab() {
  const { rings, handleRingsChange, minOrbit, highlightedId, setHighlightedId } = usePlanet();
    ...
    <div className="grid gap-3 justify-start [grid-template-columns:repeat(auto-fill,13.9em)]">
      {rings.map((r, i) => (
        <ConfigCard
          ...
          highlighted={highlightedId === r.uid}
        ...
        >
          ...
          <ColorInput
            label="Color"
            value={r.stroke}
            onChange={(v) => updateRing(r.uid, "stroke", v)}
          />
        </ConfigCard>
      ))}
